...
class Inbox(models.Model):
    ...
    * "disabled" is a bit like "deleted", but incoming mail will be deffered, not rejected
    ...
    def __str__(self):
        return "{}@{}".format(self.inbox, self.domain.domain)

    def __repr__(self):
        u_rep = six.text_type(self)
        if self.flags.deleted:
            u_rep = "%s (deleted)" % u_rep
        return smart_str('<%s: %s>' % (self.__class__.__name__, u_rep), errors="replace")
...
class Request(models.Model):
    ...
    def __str__(self):
        return "Request for {} ({})".format(self.requester, self.succeeded)
...
class Header(models.Model):
    ...
    def __str__(self):
        return six.text_type(self.name.name)
